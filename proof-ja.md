...
- そうでないとき、**bad root** を $r=\max\{\,p \mid S_p < S_{X-1} \wedge p<X-1\,\}$、**good part** を $\mathbf{G}=(S_0,\dots,S_{r-1})$、**bad part** を $\mathbf{B}=(S_r,\dots,S_{X-2})$ として
...
$\omega^0$ がちょうど1つ消える。$o(S)=H\big(\{H\{\}\}+M\big)$、
...
$$o\big(\mathbf{B}^{\,k+1}\big)=H\big(\,(k{+}1)\cdot\{\,o(\mathbf{B}_t)\,\}\,\big) \qquad(\text{補題 } \texttt{omap\_rep}).$$

第二に、$\mathbf{B}\,[m]=v\#(\mathbf{B}_t\,[m])$ では新しい $m$ は $v$ の子になる
（$\mathbf{B}_t$ の全要素は $v$ を超え、$m>v$）ので、

$$o\big(\mathbf{B}\,[m]\big)=H\big(\{\,o(\mathbf{B}_t\,[m])\,\}\big) \qquad(\text{補題 } \texttt{omap\_BfM}).$$
...
配線：bad set $\{p<X{-}1 \mid S_p<\mathrm{last}\,S\}$ は有限かつ非空なので $r=\max$ は
...
**定理（停止性, `m_termination`）。** 展開関係 $\{(T,S)\mid S\to T\}$ は整礎である。
